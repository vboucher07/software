...
@click.group("yum-tron", invoke_without_command=True)
...
@click.option(
    "--ruleset", type=click.Choice(list(AVAILABLE_RULESETS.keys())), default="yum"
)
...
@click.option("--batch-multiplier", type=click.IntRange(min=1, max=64), default=16, 
              help="Batch size multiplier for GPU efficiency (higher = larger batches)")
def train(out, ruleset, num_epochs, no_restore, objective, batch_multiplier):
    ...
    if load_path is None:
        yzt = train_a2c(yzt, num_epochs=20_000, pretraining=True, batch_size_multiplier=batch_multiplier)

    yzt = train_a2c(yzt, num_epochs=num_epochs, checkpoint_path=out, batch_size_multiplier=batch_multiplier)
